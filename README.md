...
|-------|---------------------|-------------------|------------|---------------|
| **chunkr-parse-1** | 15.86s | 0.824 | 0.775 | High-throughput workflows |
| **chunkr-parse-1-thinking** | 22.71s | 0.920 | 0.855 | Complex documents |
...
**Scores**: Levenshtein: 71.9%, TEDS: 50.3% - Both drop, but Levenshtein stays higher (structure intact).
...
**chunkr-parse-1-thinking** is now the default API model, providing the best balance of accuracy and reasoning for complex documents. Use **chunkr-parse-1** when you need maximum throughput with predictable latency for high-volume processing where speed is critical. Both models follow the same interfaces and work across the same document segments so you can mix them by route or fallback and set acceptance thresholds using either metric.
